...
def infer_source_line(
    *,
    ...
    if variable_name:
        variable_line = _find_regex_line(
            lines=lines,
...
def _find_regex_line(*, lines: list[str], pattern: str) -> int | None:
...
def _extract_variable_name(*, node_ref: str | None) -> str | None:
    ...
    if len(tokens) >= 6 and tokens[4] in {"param", "local", "member", "field"}:
        return tokens[5]
    return None
