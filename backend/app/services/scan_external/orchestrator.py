...
from .contracts import ExternalScanContext, ExternalScanResult, ExternalStageResult, ExternalStageSpec
...
def run_external_scan(
    *,
    ...
    context = build_external_scan_context(job=job, settings=settings, backend_root=backend_root)
    ...
    append_log(JobStage.AGGREGATE.value, f"读取外部结果 round={round_number}, reports_dir={context.reports_dir}")
    ...
    summary_extra: dict[str, object] = {
        ...
        "hit_rules": _safe_int(rule_summary_raw.get("hit_rules"), default=len(hit_rules)),
...
def _run_stage_command(
    *,
    ...
    try:
        ...
        duration_ms = int((time.monotonic() - started_at) * 1000)
        stdout_tail = _tail_text(_normalize_process_output(getattr(exc, "stdout", None)))
        stderr_tail = _tail_text(_normalize_process_output(getattr(exc, "stderr", None)))
        if stdout_tail:
    ...
    append_log(spec.log_stage, f"[{spec.key}] 执行完成: exit_code={result.returncode}, duration_ms={duration_ms}")
    return stage_result
...
def _load_external_audit_summary(reports_dir: Path, round_number: int) -> list[dict[str, object]]:
    summary_path = reports_dir / "audit_output" / f"round{round_number}" / "summary.json"
