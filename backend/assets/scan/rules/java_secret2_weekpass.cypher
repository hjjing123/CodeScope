MATCH
  ...
  WHERE
  NOT sourceNode:CallArg  // 排除方法入参，只检测硬编码常量
  AND NOT sourceNode.name CONTAINS ' '  // 不包含空格
AND sourceNode.name IN [
// 原有弱口令列表保留
'"123456"', '"1234"', '"root"', '"admin"', '"12345678"',
'"test"', '"1111"', '"root123"', '"root1234"', '"admin123"',
'"1qaz2wsx"', '"1qaz!QAZ"', '"asdf"', '"nacos"',
// 新增高频硬编码弱口令
'"123"', '"12345"', '"000000"', '"666666"', '"888888"',  // 纯数字
'"password"', '"passwd"', '"pwd"',  // 通用密码
'"user"', '"guest"', '"default"',  // 常见用户名作为密码
'"123abc"', '"abc123"',  // 简单组合
'"mysql"', '"oracle"', '"redis"',  // 数据库默认密码
'""'  // 空密码
]

// 密码密钥等变量（sink点）
MATCH
  (sinkNode:LocalDeclaration|FieldDeclaration|Reference)
WHERE
// 扩展变量名匹配，覆盖更多密钥类型
sinkNode.name =~ '(?i).*(secretkey|secret|accesskey|privatekey|apikey|apitoken|accesstoken|password|pass|passwd|jwtsecret|sshkey|encryptionkey|decryptionkey)$'

// 最短路径约束（最多2步关联）
MATCH
  p = shortestPath((sourceNode)-[*..2]->(sinkNode))
RETURN
