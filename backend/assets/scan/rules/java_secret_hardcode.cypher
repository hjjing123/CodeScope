// datesource.setPassword("xxxxx");
MATCH
  (sinkNode:StringLiteral)
  WHERE
  (
    ('setLoginPassword' IN sinkNode.selectors) OR
    ('setPassword' IN sinkNode.selectors) OR
    ('setPass' IN sinkNode.selectors) OR
    ('setAccessKeySecret' IN sinkNode.selectors) OR
    ('setSecretKey' IN sinkNode.selectors) OR
    //DriverManager.getConnection(url, "sa", "");
    (('getConnection' IN sinkNode.selectors) AND ('DriverManager' IN sinkNode.receivers) AND sinkNode.argPosition = 2)
  )

//  AND
//  NOT sinkNode.name  IN ['\"AES\"', '\"DES\"', '\"MD5\"']

RETURN
DISTINCT sinkNode AS path
