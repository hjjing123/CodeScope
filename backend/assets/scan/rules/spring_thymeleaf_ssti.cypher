// Spring Thymeleaf SSTI (full-scene generic structure):
// A) source -> Thymeleaf process call
// B) source -> Method(with Thymeleaf evidence) -> ReturnArg:String
// Adaptive fallback: when Thymeleaf call evidence is absent in graph, use method-name/fullName hint.
...
MATCH (sourceNode:SpringControllerArg)
WHERE
  NOT coalesce(sourceNode.type, '') IN ['Long', 'Integer', 'HttpServletResponse', 'int', 'long']
