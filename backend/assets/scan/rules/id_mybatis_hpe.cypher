...
MATCH
  ...
  WHERE
  // 数据库查询 mybatis
  ('selectAll' IN sinkNode.selectors AND 'SqlRunner' IN sinkNode.receiverTypes) OR
  //  ('selectOne' IN sinkNode.selectors AND   'SqlRunner' IN  sinkNode.receiverTypes)
  ('selectOne' IN sinkNode.selectors) OR
  ('selectById' IN sinkNode.selectors) OR
  ('selectBatchIds' IN sinkNode.selectors) OR
  sinkNode:MybatisMethodArg
MATCH
