...
MATCH
  ...
  WHERE
  // 腾讯云对象存储上传 cos
  ('uploadFile' IN sinkNode.selectors AND  'COSClient' IN sinkNode.receiverTypes) OR
  // 阿里云对象存储上传 oss
  ('putObject' IN sinkNode.selectors AND  'OSSClient' IN sinkNode.receiverTypes) OR
  // 七牛云对象存储
  ...
  // 微软 Azure
  ('createFrom' IN sinkNode.selectors AND  'BlobClient' IN sinkNode.receiverTypes) OR
  // gcp
  ('createFrom' IN sinkNode.selectors AND  'Storage' IN sinkNode.receiverTypes) OR
  ('create' IN sinkNode.selectors AND  'Storage' IN sinkNode.receiverTypes)
...
对象存储是一种存储大量数据的方法，它允许通过网络访问和管理数据。与传统的文件系统存储不同，对象存储将数据封装在“对象”中，每个对象包含数据和元数据（如名称、创建日期等）。对象存储服务通常由第三方提供，如Amazon S3、Google Cloud Storage和Microsoft Azure Blob Storage，它们提供可扩展性、持久性和高可用性。
