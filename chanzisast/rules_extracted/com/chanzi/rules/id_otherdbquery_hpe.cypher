...
// flatArgs 是入参类型dto 展开后的参数名列表，字符串数组，判断展开后的参数是否包含订单id参数
any(
...
MATCH
  ...
  WHERE
  // 数据库查询 hibernate
  ('createQuery' IN sinkNode.selectors AND 'Session' IN sinkNode.receiverTypes) OR
  ('createSQLQuery' IN sinkNode.selectors AND   'Session' IN  sinkNode.receiverTypes) OR
  ('createNativeQuery' IN sinkNode.selectors AND   'QueryProducer' IN  sinkNode.receiverTypes) OR
  ('createQuery' IN sinkNode.selectors AND   'QueryProducer' IN  sinkNode.receiverTypes) OR
  ('createSQLQuery' IN sinkNode.selectors AND   'QueryProducer' IN  sinkNode.receiverTypes) OR
  ('createQuery' IN sinkNode.selectors AND   'EntityManager' IN  sinkNode.receiverTypes) OR
  ('createNativeQuery' IN sinkNode.selectors AND   'EntityManager' IN  sinkNode.receiverTypes) OR
  // spring data dao
  ('findOne' IN sinkNode.selectors )
//  ('findAll' IN sinkNode.selectors ) OR
MATCH
