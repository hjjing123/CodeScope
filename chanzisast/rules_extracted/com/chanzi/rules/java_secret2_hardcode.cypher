...
// source 点，字符串常量，不是方法入参， 不包含空格
MATCH
  (sourceNode:StringLiteral)
  WHERE NOT sourceNode:CallArg AND NOT sourceNode.name CONTAINS ' ' AND
  NOT sourceNode.name  IN ['"AES"', '"DES"', '"MD5"', '"UTF-8"', '""']

// sink点， 密码密钥等变量
MATCH
  (sinkNode:LocalDeclaration|FieldDeclaration|Reference)
WHERE
sinkNode.name =~ '(?i).*(secretkey|accesskey|privatekey|password|pass|passwd)$'
